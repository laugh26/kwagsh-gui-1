...
int main(int argc, char* argv[]) {
	...
	//as root
	args << "-c" << "printf '[Desktop Entry]\\nName = KWAGSH URL Handler\\nGenericName = KWAGSH\\nComment = Handle URL Sheme kgh://\\nExec = " + QCoreApplication::applicationFilePath() + " %%u\\nTerminal = false\\nType = Application\\nMimeType = x-scheme-handler/kgh;\\nIcon = KWAGSH-Wallet' | tee /usr/share/applications/kgh-handler.desktop";
	...
	if (!QDir().exists(dataDirPath)) {
		QString oldDataDirPath = QString(dataDirPath).replace("kwagsh", "kwa");
		if (QDir().exists(oldDataDirPath)) {
			auto dir = QDir(oldDataDirPath);
			QLockFile oldLockFile(dir.absoluteFilePath("kwawallet.lock"));
			...
			if (QDir().rename(oldDataDirPath, dataDirPath)) {
				...
				std::vector<QString> files = {"kwawallet.cfg", "kwawallet.log", "kwawallet.wallet", "kwawallet.wallet.backup", "kwawallet.lock"};
				for (auto f : files) QFile().rename(dir.absoluteFilePath(f), dir.absoluteFilePath(f).replace("kwa", "kwagsh"));
			} else {
	...
	if (Settings::instance().isTestnet()) {
		...
		ts_label->setText("<a style='text-decoration:none; color: red;' href='http://txt.kwagsh.xyz'>TESTNET</a>");
...
#ifdef _WIN32
		if (!QCoreApplication::applicationFilePath().toLower().contains("kwagsh/kwagsh-wallet")) {
