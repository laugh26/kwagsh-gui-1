...
void SendFrame::parsePaymentRequest(QString _request) {
    ...
    if(_request.startsWith("KGH://", Qt::CaseInsensitive))
    {
       _request.replace(0, 7, "KGH:");
    }
    if(!_request.startsWith("KGH:", Qt::CaseInsensitive)) {
      QCoreApplication::postEvent(&MainWindow::instance(), new ShowMessageEvent(tr("Payment request should start with KGH:"), QtCriticalMsg));
      return;
    }
